...
type AnswerToolResponse = AskResult & { warning?: string };

function toToolResult(result: AskResult) {
    const tool_response: AnswerToolResponse = { ...result };

    if (toolConfig.mode === ChatbotMode.Base) {
        tool_response.warning =
 ...
 */
function createElicitationHandler(): ConfirmationHandler | undefined {
...
server.registerTool(
    'veeam-question-answering',
    {
        ...
        annotations: {
            readOnlyHint: !toolConfig.actionsEnabled,
            destructiveHint: false,
            openWorldHint: false,
        },
    },
    async ({ question }) => {
        const result = await answerQuestion(question, { confirmationHandler: createElicitationHandler() });
        return toToolResult(result);
    },
);

if (toolConfig.actionsEnabled) {
    server.registerTool(
        'veeam-confirm-action',
        {
        ...
        async ({ action_id, approve }) => {
            const result = await confirmAction(action_id, approve, { confirmationHandler: createElicitationHandler() });
            return toToolResult(result);
        },
    );

    server.registerTool(
        'veeam-list-pending-actions',
        {
...
log.info(`MCP server started (mode: ${toolConfig.mode}, actions: ${toolConfig.actionsEnabled ? 'enabled' : 'disabled'})`);
